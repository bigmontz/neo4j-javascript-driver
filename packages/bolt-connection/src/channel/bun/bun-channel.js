 ...
 * @access private
 */
export default class BunChannel {
  /**
   * Create new instance
  ...
  constructor(
    ...
    connect(config, {
      ...
      data: (_, data) => {
        ...
        if (this.onmessage) {
          this.onmessage(new ChannelBuffer(data))
        }
      },
      error: (_, error) => {
      ...
      close: () => {
        this._handleConnectionTerminated()
      }
    }).then(conn => {
...
const TrustStrategy = {
  TRUST_CUSTOM_CA_SIGNED_CERTIFICATES: async function (config, socket) {
    if (
  ...
  TRUST_SYSTEM_CA_SIGNED_CERTIFICATES: function (config, socket) {
    return Bun.connect({
      hostname: config.address.resolvedHost(),
      port: config.address.port(),
      tls: true,
...
async function _connect(config, socket) {
  ...
  if (trustStrategy != null) {
    return await trustStrategy(config, socket)
  }
