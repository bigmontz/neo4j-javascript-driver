...
import createHandlers from './request-handlers.js';

const listener = Deno.listen({ port: 9876 });
let index = 0;
const contexts = new Map<number, Context>();
const requestHandlers: { [key: string]: (c: Context, data: any, wire: any) => void } = createHandlers(neo4j, null, { });
...
const backend: Backend = {
  openContext: (contextId) => {
    ...
    console.log(getShouldRunTest)
    contexts.set(contextId, new Context(getShouldRunTest));
  },
  ...
  handle: (contextId, conn, req) => {
    ...
    handler(contexts.get(contextId)!!, data, {
      writeResponse: (name: string, data: object|null) => write(conn, { name, data }),
      writeError: (e: Error) => {
        if (e.name && e instanceof neo4j.Neo4jError) {
          const id = contexts.get(contextId)!!.addError(e)
          write(conn, { name: 'DriverError', data: { id, message: e.message + ' (' + e.code + ')', code: e.code } })
          return
        }
