import binders from './cypher-native-binders.js'

export default function (neo4j, ResultObserver, tls) {
  const { cypherToNative, nativeToCypher } = binders(neo4j);
  ...
  function NewDriver (context, data, wire) {
    const {
      uri,
  ...
  function DriverClose (context, data, wire) {
    const { driverId } = data
  ...
  function NewSession (context, data, wire) {
  ...
  function SessionClose (context, data, wire) {
  ...
  function SessionRun (context, data, wire) {
  ...
  function ResultNext (context, data, wire) {
    const { resultId } = data
  ...
  function ResultConsume (context, data, wire) {
    const { resultId } = data
    const resultObserver = context.getResultObserver(resultId)
    resultObserver
      .completitionPromise()
  ...
  function ResultList (context, data, wire) {
    const { resultId } = data
  ...
  function SessionReadTransaction (context, data, wire) {
  ...
  function TransactionRun (context, data, wire) {
  ...
  function RetryablePositive (context, data, wire) {
    const { sessionId } = data
  ...
  function RetryableNegative (context, data, wire) {
  ...
  function SessionBeginTransaction (context, data, wire) {
  ...
  function TransactionCommit (context, data, wire) {
    const { txId: id } = data
  ...
  function TransactionRollback (context, data, wire) {
    const { txId: id } = data
    const { tx } = context.getTx(id)
    tx.rollback()
      .then(() => wire.writeResponse('Transaction', { id }))
  ...
  function SessionLastBookmarks (context, data, wire) {
    const { sessionId } = data
    const session = context.getSession(sessionId)
  ...
  function SessionWriteTransaction (context, data, wire) {
    const { sessionId, txMeta: metadata } = data
    const session = context.getSession(sessionId)
    session
      .writeTransaction(
  ...
  function StartTest (context, { testName }, wire) {
  ...
  function GetFeatures (_context, _params, wire) {
  ...
  function VerifyConnectivity (context, { driverId }, wire) {
    const driver = context.getDriver(driverId)
    driver
      .verifyConnectivity()
  ...
  function CheckMultiDBSupport (context, { driverId }, wire) {
    const driver = context.getDriver(driverId)
    driver
      .supportsMultiDb()
      .then(available =>
  ...
  function GetRoutingTable (context, { driverId, database }, wire) {
  ...
  function ForcedRoutingTableUpdate (context, { driverId, database, bookmarks }, wire) {
    const driver = context.getDriver(driverId)
